...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    //todo convert to query for cs use
    $student_ref_num = $_SESSION['ref_num'];
    ...
    if ($result && $result->num_rows > 0) {
        ...
        //todo encoder here is student
        $sql = "INSERT INTO $bookingtable (`ref_num`, `schedule_ref_num`, `student_ref_num`, `encoded_by`) 
                VALUES ('$new_ref_num', '$schedule_ref_num', '$student_ref_num', '$student_ref_num - $presentdate');";
        ...
        if ($conn->query($sql)) {
            ...
            if ($mailToStudent && $mailToTeacher) {
                echo "<script type='text/javascript'>alert('Booking added successfully!'); window.location.href='class.php';</script>";
            } else {
            ...
            echo "<script type='text/javascript'>alert('Error: $conn->error'); window.location.href='class.php';</script>";
        }
    } else {
        echo "<script type='text/javascript'>alert('Invalid schedule, please try again'); window.location.href='class.php';</script>";
    }
